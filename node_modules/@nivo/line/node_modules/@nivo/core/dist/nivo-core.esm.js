import React, { createContext, useContext, memo, useMemo, useRef, useState, useCallback, Component, PureComponent } from 'react';
...
var memoizedColorModifier = memoize(function (method, _amount) {
  ...
  return memoize(function (d) {
    return rgb(d.color)[method](amount)
    .toString();
  }, function (d) {
    return d.color;
  });
...
var withHierarchy = (function () {
  ...
  return compose(defaultProps(_defineProperty$d({}, valueKey, valueDefault)), setPropTypes((_setPropTypes = {}, _defineProperty$d(_setPropTypes, srcKey, PropTypes.object.isRequired), _defineProperty$d(_setPropTypes, valueKey, PropTypes.oneOfType([PropTypes.string, PropTypes.func]).isRequired), _setPropTypes)), withPropsOnChange([srcKey, valueKey], function (props) {
    return _defineProperty$d({}, destKey, hierarchy(props[srcKey]).sum(getAccessorFor(props[valueKey])));
  }));
});
...
var Container$1 = function Container(_ref) {
  ...
  return React.createElement(themeContext.Provider, {
    ...
    ref: containerEl
  }, children, isTooltipVisible && React.createElement("div", {
...
var Axis = function Axis(_ref2) {
  ...
  if (_legend !== undefined) {
    ...
    legend = React.createElement("text", {
  ...
  var tickElements;
  if (!animate) {
    tickElements = React.createElement("g", null, ticks.map(function (tick) {
      return React.createElement(AxisTick, _extends$2({
        key: tick.key,
        value: tick.key,
        format: format,
        lineX: tick.lineX,
    ...
    tickElements = React.createElement(TransitionMotion, {
      willEnter: willEnter,
  ...
  return React.createElement("g", {
    transform: "translate(".concat(x, ",").concat(y, ")")
  }, legend, tickElements, React.createElement("line", {
...
var enhance = compose(withMotion(), withPropsOnChange(['format', 'scale'], function (_ref4) {
  var format$1 = _ref4.format,
      scale = _ref4.scale;
  if (!format$1 || isFunction(format$1)) {
    return {
      format: format$1
    };
...
var Axes = function Axes(_ref) {
  ...
  return React.createElement("g", null, positions.map(function (position) {
    var axis = axes[position];
    if (!axis) return null;
    var scale = horizontalPositions$1.includes(position) ? xScale : yScale;
    return React.createElement(Axis$1, _extends$3({
      theme: theme
    }, axis, {
      key: position,
      width: width,
    ...
    value: function render() {
      ...
      if (!animate) {
        return React.createElement("g", null, lines.map(function (line) {
          return React.createElement(GridLine, _extends$4({
            key: line.key
...
var CartesianMarkers = function CartesianMarkers(_ref) {
  ...
  return React.createElement("g", null, markers.map(function (marker, i) {
    return React.createElement(CartesianMarkersItem$1, _extends$5({
      key: i
    }, marker, {
      width: width,
      height: height,
...
export { Axes$1 as Axes, Axis$1 as Axis, BasicTooltip, CartesianMarkers$1 as CartesianMarkers, CartesianMarkersItem$1 as CartesianMarkersItem, Chip, Container, Defs, DotsItem$1 as DotsItem, DotsItemDefaultProps, Grid$1 as Grid, LinearGradient, PatternDots, PatternLines, PatternSquares, ResponsiveWrapper, SmartMotion, SvgWrapper, TWO_PI, TableTooltip, absoluteAngleDegrees, absoluteAngleRadians, alignBox, areaCurvePropKeys, areaCurvePropType, axisThemePropType, bindDefs, blendModePropType, boxAlignments, closedCurvePropKeys, closedCurvePropType, colorInterpolatorIds, colorInterpolators, colorMotionSpring, colorSchemeIds, colorSchemes, computeArcBoundingBox, computeAxisTicks, computeGridLines, curveFromProp, curvePropKeys, curvePropMapping, curvePropType, defaultAnimate, defaultCategoricalColors, defaultColorRange, defaultMargin, defaultMotionDamping, defaultMotionStiffness, defaultTheme, defsMapping, defsPropTypes, degreesToRadians, dotsThemePropType, extendDefaultTheme, getAccessorFor, getAccessorOrValue, getAngle, getColorRange, getColorScale, getColorsGenerator, getDistance, getHoveredArc, getInheritedColorGenerator, getInterpolatedColor, getLabelGenerator, getPolarLabelProps, getRelativeCursor, gradientTypes, gridThemePropType, guessQuantizeColorScale, isCursorInRect, isCursorInRing, isMatchingDef, labelsThemePropType, legendsThemePropType, lineCurvePropKeys, lineCurvePropType, linearGradientDef, marginPropType, markersThemePropType, midAngle, motionPropTypes, nivoCategoricalColors, noop, patternDotsDef, patternLinesDef, patternSquaresDef, patternTypes, positionFromAngle, quantizeColorScalePropType, quantizeColorScales, quantizeColorScalesKeys, radiansToDegrees, renderAxesToCanvas, renderAxisToCanvas, renderGridLinesToCanvas, scalePropType, stackOffsetFromProp, stackOffsetPropKeys, stackOffsetPropMapping, stackOffsetPropType, stackOrderFromProp, stackOrderPropKeys, stackOrderPropMapping, stackOrderPropType, textPropsByEngine, themeContext, themePropType, tooltipContext, treeMapTileFromProp, treeMapTilePropKeys, treeMapTilePropMapping, treeMapTilePropType, useDimensions, usePartialTheme, useTheme, useTooltip, withColors, withContainer, withCurve, withDimensions, withHierarchy, withMotion, withTheme };
